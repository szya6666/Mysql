...
```
先安装依赖  
	apt-get install libpcre3-dev gcc zlib1g-dev  build-essential
```
...
- nginx站点配置目录：/etc/nginx/
  - 其中主配置为nginx.conf
  - sites-available子目录存放站点配置文件，其中有一个default文件为虚拟站点的配置模板，自己的虚拟站点可以以此为模板进行配置。多个域名可以解析到一个ip上    域名要访问的话必须得备案 
...
server{
    listen 80;
    server_name www.91.com 91.com;

    root /var/www/html/91;
...
| 角色        | ip           | 作用                              |
| ----------- | ------------ | --------------------------------- |
| lvs负载均衡 | 10.8.13.42   | 请求分担 将域名解析到这一台服务器 |
| web01       | 10.8.152.117 | 轮询的web服务器01                 |
| web02       | 10.8.152.95  | 轮询的web服务器01                 |
...
1. 给 web01 web02  
   ...
   倒数第二行 加上 include /usr/local/nginx/conf/vhost/*.conf; 
   
   server{
       listen 80;
       server_name www.91.com 91.com;
   
       root /usr/local/nginx/html/91;
...
~~~
   #连接池
   upstream tianshangrenjian {  #www_server_pools自定义的连接池名称   这里是什么名字
      server 10.8.152.117 weight=2;    #连接的服务器，可以ip或者是域名
      server 10.8.152.95 weight=1; 值越大权重越高
   }   
...
- ip_hash  根据客户端ip哈希的结果确定访问那个服务器。 保证  请求 由固定的服务器响应  
